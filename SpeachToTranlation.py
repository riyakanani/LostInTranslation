...
def main():
    ...
    while True:
        try:
            ...
            numeric_parts = re.findall(r'\d+', taxi)
            if(numeric_parts == 0):  
                taxiWords = taxi.split()
                for i in range(len(taxiWords) - 1):
                    try:
                ...
                single_digit_array = []
                # Iterate over each numeric part
                for num in numeric_parts:
                    # Convert the number to a string and then split it into single digits
                    digits = [int(digit) for digit in str(num)]
                    
                    # Extend the single_digit_array with the single digits
                    single_digit_array.extend(digits)
                taxiWords = single_digit_array
            

            np.random.shuffle(taxiWords)
            taxi = ""
            taxiWords = [str(item) for item in taxiWords]
            taxi = ''.join(taxiWords)
